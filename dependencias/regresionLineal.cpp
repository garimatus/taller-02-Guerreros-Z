...
regresionLineal::regresionLineal(long int n, std::vector<long long int> datos_x, std::vector<long long int> datos_y, const char* variable)
{
    this -> X = datos_x;
    this -> Y = datos_y;
    this -> muestra = n;
    ...
    this -> prom_x /= double(n);
    this -> prom_y /= double(n);
...
void regresionLineal::setExpresion(std::string variable)
{
    this -> x = variable;
...
std::string regresionLineal::getExpresion()
{
    return this -> expresion;
}

long double regresionLineal::getR()
{
    return this -> R2;
}
